...
import br.com.finalcraft.richpresence.LauncherRichPresence;
...
public class LaunchSupervisor {
    ...
    private void launch(Window window, final Instance instance, Session session, final LaunchListener listener) {
        ...
        Futures.addCallback(processFuture, new FutureCallback<Process>() {
            ...
            public void onSuccess(Process result) {
                ...
                LauncherRichPresence.onInstanceStart(instance.getTitle());
            }
        ...
        future.addListener(new Runnable() {
            ...
            public void run() {
                ...
                LauncherRichPresence.onMinecraftClose();
            }
        }, sameThreadExecutor());
    }
}
